...
Fitur / Menu:
Halaman Utama (Landing Page): Informasi awal mengenai fungsionalitas aplikasi dan navigasi utama.

Autentikasi Pengguna: Registrasi akun baru dan login aman dengan validasi data instan.

Panel Kontrol (Dashboard): Widget statistik penggunaan layar, durasi sesi, dan indikator status beban kerja mata.

Sesi Pelacakan Aktif (Tracking Screen Time): Timer digital real-time dengan sistem peringatan warna dinamis (Hijau/Kuning/Merah) berdasarkan ambang batas medis aman.

Cek Kesehatan Mata: Kuesioner skrining mandiri untuk mendeteksi tingkat kelelahan mata pengguna.

Hasil Analisis & Rekomendasi: Skor indeks kesehatan mata, status klinis, dan instruksi terapi mandiri otomatis (seperti metode 20-20-20).

Laporan Riwayat (History Tracking): Grafik batang dinamis (Chart.js) dan tabel kronologis untuk memantau tren penggunaan layar mingguan.

Manajemen Profil: Pembaruan data personal (Nama & Email) serta kustomisasi avatar profil secara interaktif.

Konfirmasi Keluar (Logout Security): Jendela pop-up pengaman (SweetAlert2) untuk mengakhiri sesi dan mengamankan integritas data medis.
